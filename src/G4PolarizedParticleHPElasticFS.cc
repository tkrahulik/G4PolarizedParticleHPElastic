...
void G4PolarizedParticleHPElasticFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String &, G4ParticleDefinition* )
  {
    G4String tString = "/FS";
    G4bool dbool;
...
//    cout << "@@@" << the3Target<<G4endl;
    ...
    G4double theAsymmetry = 2.0; //Initiliaze polarization asymmetry (initialize as max)
    G4double theSampleAsym = 2.1; //Initiliaze variable for random sampling
    G4int theCounter = 0;
    /*072017: Create while loop for random sampling to
     * weight cross section by polarization asymmetry 
     * ~T.Krahulik*/
    while((theSampleAsym > theAsymmetry) && (theCounter < 100))
      {
	...
	if (frameFlag == 1) // final state data given in target rest frame.
	  {
	    ...
	    /*===================================================*/
	    /*=  Calculate Polarization Asymmetry Ay(E, theta)  =*/
	    /*=---------------072017 ~T.Krahulik----------------=*/
	    /*===================================================*/

	    G4int targetA = const_cast<G4int>( theTrack->GetMaterial()->GetA() );
	    G4int targetZ = const_cast<G4int>( theTrack->GetMaterial()->GetZ() );
	    G4ParticleDefinition* scatteredParticle = theNeutron->GetDefinition();

	    //Evaluate analyzing power based on reaction, energy, and scattering angle
	    G4double theAnalyzingPower = GetAnalyzingPower(targetZ, targetA, scatteredParticle, eKinetic, theta, "Elastic");

	    // Normalize the momentum vectors of the incident and final state neutron
	    G4ThreeVector the3NeutronDir = the3Neutron / nEnergy;
	    G4ThreeVector tempVectorDir = tempVector / en;
	    // Calculate unit vector normal to reaction plane
	    G4ThreeVector thePolVectorDir = the3NeutronDir.cross( tempVectorDir );
	    // Obtain the polarization vector of the incident particle
	    G4ThreeVector thePolVector = incidentParticle->GetPolarization();
	    G4double thePolVectorMag = thePolVector.dot( thePolVectorDir );


	    theAsymmetry = 1 + thePolVectorMag*theAnalyzingPower;
	    /*===================================================*/
	...
	theSampleAsym = 2*G4UniformRand(); //Resample
	theCounter++;
      } //072017: End of loop for random sampling to weight cross section by polarization asymmetry ~T.Krahulik
