...
G4HadProjectile::G4HadProjectile() 
{
  ...
  theTime = 0.0;
  theBoundEnergy = 0.0;
  thePol = 0; //180717: Polarization Vector ~T.Krahulik
}
...
G4HadProjectile::G4HadProjectile(const G4DynamicParticle &aT) 
  : theMat(NULL),
    ...
    thePol(aT.GetPolarization()) //180717: Get Polarization from G4DynamicParticle ~T.Krahulik
{
...
void G4HadProjectile::Initialise(const G4Track &aT)
{
  ...
  theDef = aT.GetDefinition();
  thePol = aT.GetDynamicParticle()->GetPolarization(); //180717: Get Polarization from G4DynamicParticle ~T.Krahulik
