...
#include "G4Threading.hh"
  ...
  G4PolarizedParticleHPElastic::G4PolarizedParticleHPElastic()
    :G4HadronicInteraction("PolarizedNeutronHPElastic")
  ,theElastic(NULL)
  ,numEle(0)
  {
    overrideSuspension = false;
/*
    G4PolarizedParticleHPElasticFS * theFS = new G4PolarizedParticleHPElasticFS;
    if(!getenv("G4NEUTRONHPDATA")) 
       throw G4HadronicException(__FILE__, __LINE__, "Please setenv G4NEUTRONHPDATA to point to the neutron cross-section files.");
    dirName = getenv("G4NEUTRONHPDATA");
    G4String tString = "/Elastic";
    dirName = dirName + tString;
//    G4cout <<"G4PolarizedParticleHPElastic::G4PolarizedParticleHPElastic testit "<<dirName<<G4endl;
...
*/
  ...
  G4HadFinalState * G4PolarizedParticleHPElastic::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
...
const std::pair<G4double, G4double> G4PolarizedParticleHPElastic::GetFatalEnergyCheckLevels() const
{
...
G4int G4PolarizedParticleHPElastic::GetVerboseLevel() const 
{
   return G4ParticleHPManager::GetInstance()->GetVerboseLevel();
}
void G4PolarizedParticleHPElastic::SetVerboseLevel( G4int newValue ) 
{
   G4ParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void G4PolarizedParticleHPElastic::BuildPhysicsTable(const G4ParticleDefinition&)
{
...
void G4PolarizedParticleHPElastic::ModelDescription(std::ostream& outFile) const
{
